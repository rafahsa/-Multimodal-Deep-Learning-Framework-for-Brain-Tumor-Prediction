#!/usr/bin/env python3
"""
ResNet50-3D Training Script for BraTS 2018

This script trains a ResNet50-3D model for brain tumor classification (HGG vs LGG)
using MedicalNet pretrained weights, CrossEntropyLoss, and multi-GPU support.
...
def main():
    parser = argparse.ArgumentParser(description="Train ResNet50-3D model")
    ...
    parser.add_argument('--pretrained-path', type=str, default=None,
                       help='Path to MedicalNet pretrained weights (optional)')
    parser.add_argument('--dropout', type=float, default=0.4,
                       help='Dropout rate (default: 0.4, reduced for better feature learning)')
    ...
    parser.add_argument('--lr', type=float, default=1e-4,
                       help='Learning rate for backbone (default: 1e-4 for fine-tuning pretrained)')
    parser.add_argument('--classifier-lr', type=float, default=None,
                       help='Learning rate for classifier head (default: 2x backbone LR)')
    ...
    parser.add_argument('--output-dir', type=str, default='results/ResNet50-3D',
                       help='Output directory (default: results/ResNet50-3D)')
    ...
    logger.info(f"Starting ResNet50-3D training for fold {args.fold}")
    ...
    # Create model
    logger.info("Creating ResNet50-3D model...")
    logger.info("Model architecture: ResNet50-3D (MedicalNet-compatible)")
    
    # Create model with MedicalNet pretrained weights if provided
    logger.info(f"Creating ResNet50-3D with {in_channels} input channels")
    model = ResNet50_3D(
        num_classes=2,
        in_channels=in_channels,
        pretrained_path=args.pretrained_path,
        dropout=args.dropout,
        logger=logger,  # Pass logger for detailed pretrained weight loading logs
        adapt_multimodal=(in_channels > 1)  # Auto-adapt if multi-modal
    )
    ...
    # Differential learning rates for pretrained backbone vs new classifier
    # This is critical for fine-tuning: backbone needs lower LR to preserve pretrained features
    # Reduced classifier LR multiplier from 10x to 2x for stability
    classifier_lr = args.classifier_lr if args.classifier_lr is not None else args.lr * 2
    
    # Separate parameter groups
    backbone_params = list(model.get_backbone_params())
    classifier_params = list(model.get_classifier_params())
    
    logger.info(f"Backbone parameters: {sum(p.numel() for p in backbone_params)/1e6:.2f}M")
    logger.info(f"Classifier parameters: {sum(p.numel() for p in classifier_params)/1e6:.2f}M")
    logger.info(f"Backbone LR: {args.lr:.2e}, Classifier LR: {classifier_lr:.2e} (2x for stable adaptation)")
    
    # Freeze backbone if requested
    if args.freeze_backbone_epochs > 0:
        logger.info(f"Freezing backbone for first {args.freeze_backbone_epochs} epochs")
        for param in backbone_params:
            param.requires_grad = False
    
    # Optimizer with differential learning rates
    if args.optimizer == 'adam':
        # Adam with different LRs for backbone and classifier
    ...
    if args.scheduler == 'cosine':
        ...
        def lr_lambda_backbone(epoch):
            """LR schedule for backbone (pretrained) - more conservative"""
            if epoch < warmup_epochs:
                ...
                # More conservative decay for pretrained backbone
    ...
    if args.ema_decay > 0:
        ...
        ema_model = ResNet50_3D(
            num_classes=2,
            in_channels=in_channels,
            pretrained_path=None,  # EMA model doesn't need pretrained weights
            dropout=args.dropout,
            logger=logger,
            adapt_multimodal=(in_channels > 1)  # Match main model's adaptation
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Unfreeze backbone if freeze period ended
