#!/usr/bin/env python3
"""
Train Logistic Regression Meta-Learner with ROI MIL

This script trains a meta-learner using the ROI MIL variant ensemble OOF predictions.
It is identical to train_meta_learner.py except it uses merged_oof_predictions_roi_mil.csv.

Usage:
    python scripts/ensemble/train_meta_learner_roi_mil.py --threshold 0.22
"""
...
# Paths - NEW directories for ROI MIL variant
MERGED_OOF_FILE = Path('ensemble/oof_predictions/merged_oof_predictions_roi_mil.csv')
ENSEMBLE_DIR = Path('ensemble')
MODELS_DIR = ENSEMBLE_DIR / 'models' / 'roi_mil'
RESULTS_DIR = ENSEMBLE_DIR / 'results' / 'meta_learner_roi_mil'
METRICS_FILE = RESULTS_DIR / 'meta_learner_metrics.json'
MODEL_FILE = MODELS_DIR / 'meta_learner_logistic_regression_roi_mil.joblib'
...
def train_meta_learner(X: np.ndarray, y: np.ndarray, use_class_weights: bool = True) -> LogisticRegression:
    ...
    """
    logger.info("\n" + "="*80)
    logger.info("Training Logistic Regression Meta-Learner (ROI MIL Variant)")
    logger.info("="*80)
    ...
    model = LogisticRegression(
        random_state=42,  # Same seed as baseline
        max_iter=1000,
        solver='lbfgs',
...
def save_model(model: LogisticRegression, metrics: Dict, save_model_file: bool = True):
    ...
    if 'y_pred_proba' in metrics_serializable:
        del metrics_serializable['y_pred_proba']
    if 'y_pred' in metrics_serializable:
        del metrics_serializable['y_pred']
    if 'y_true' in metrics_serializable:
        del metrics_serializable['y_true']
    ...
    metrics_serializable['metadata'] = {
        ...
        'threshold': float(threshold),
        'dataset': 'OOF predictions with ROI MIL (validation data from cross-validation)',
    ...
    # Also save to main metrics file
    with open(METRICS_FILE, 'w') as f:
        json.dump(metrics_serializable, f, indent=2)
    logger.info(f"✓ Saved main metrics to: {METRICS_FILE}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train Logistic Regression Meta-Learner with ROI MIL',
    ...
    logger.info("=" * 80)
    logger.info("Logistic Regression Meta-Learner Training (ROI MIL Variant)")
    logger.info("=" * 80)
    ...
    try:
        ...
        metrics_file = save_model(model, metrics, save_model_file=True)
